...
| 2. First complete event experience | Make one caregiver event journey real enough to build against. | Clinic resident overview, event list/detail, priority, uncertainty, acknowledge/check/resolve, and feedback flows on mocks; establish the separate home mock foundation without clinic operations. | Durable room/resident/event/feedback history and the first Product API around the agreed journey. | Product-facing device states: online, offline, poor quality, buffering, retrying, assignment unavailable. | A caregiver can complete the same event journey in frontend mocks and backend rules without contradicting each other; clinic and home mocks remain separate. |
...
| 5. Simulated telemetry and ingestion | Let a realistic simulated room safely enter and move through the system. | Scenario views for normal activity, device trouble, uncertainty, and recovery. | Ingest, persist, validate, deduplicate, and replay simulated signals; handle delay, missing input, and connectivity changes. | Refine realistic simulated output and quality signals. | A room scenario is safely ingested and replayable with honest monitoring/device status; full event intelligence is the next phase. |
...
| 7. Home/family product | Turn the early home mock foundation into a complete separate family experience. | Home view focused on “are they okay?” rather than clinic workflow, using mature monitoring data and family-safe language. | Appropriate, permissioned view of the same core information. | No new product dependency. | Clinic and family users receive safe, purpose-built views. |
...
**Frontend scope:** build the clinic flow and the separate home mock foundation against contract-valid mocks behind a replaceable data client. Do not wait for the real API. The full home/family product remains Phase 7.
