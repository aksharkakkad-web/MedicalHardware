  ...
  return <section className={styles.page}>
    ...
    <div className={styles.layout}>
      ...
      <aside className={styles.sideColumn}>
        ...
        <section className={styles.panel}><div className={styles.panelHeader}><h2>Monitoring</h2><StatusPill label={resident.device.status} tone={deviceTone[resident.device.status]} /></div><dl><div><dt>State</dt><dd>{resident.monitoring.state}</dd></div><div><dt>Device</dt><dd>{resident.device.label}</dd></div><div><dt>Last update</dt><dd>{formatTime(resident.monitoring.lastUpdatedAt)}</dd></div></dl>{resident.monitoring.contextLabel && <p className={styles.context}>{resident.monitoring.contextLabel}</p>}<Link className={styles.setupLink} href={`/residents/${resident.residentId}/setup`}>Review monitoring setup <ArrowIcon /></Link></section>
      </aside>
    </div>
  </section>;
}
